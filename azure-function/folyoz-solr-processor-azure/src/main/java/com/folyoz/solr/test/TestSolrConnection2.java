...
public class TestSolrConnection2 {
	public static void main(String[] z) throws SolrServerException, IOException {
		 ...
		 * PASSWORD); server.getHttpClient().getState().setCredentials(new
		 * AuthScope(HOSTNAME, 80, AuthScope.ANY_REALM), defaultcreds);
		 * server.getHttpClient().getParams().setAuthenticationPreemptive(true);
		 * SolrInputDocument doc1 = new SolrInputDocument();
		 * 
		 * //client = new SolrClient.Builder().withSolrUrl(
		 * "http://folyozsolr.southcentralus.cloudapp.azure.com:8983/solr/").
		 * withHttpClient(getHttpClient()).build(); // client.connect();
